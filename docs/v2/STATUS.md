...
research-only paths with explicit workloads, live collectors and a CNDEV adapter.
...
| Active self-test | Allocated-buffer copy and vector-add patterns plus matrix multiplication with correctness checks | Explicit execution only; NVIDIA/AMD torch and modern torch_mlu paths; partial memory coverage |
...
| Alibaba PPU | Inventory/driver/memory plus utilization, temperature, power and clocks through documented SAIL queries; ECC evidence and generation-specific codes | SAIL v2.1.1 documented formats; fixture-tested, real cards not yet tested; active SAIL compute runtime remains gated |
...
change description: **179 tests pass locally**, with successful wheel/sdist builds
...
that test checks the bridge protocol, not the real vendor ABI or hardware.
...
3. SAIL compute runtime/API evidence before enabling PPU active probes; the
   existing code returns INCONCLUSIVE instead of running on a possibly unrelated
   CUDA device. PPU management metrics/diagnostics/topology are implemented.
